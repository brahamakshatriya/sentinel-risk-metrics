...
function LineSeriesStroke({
  ...
  /* Phase 4 local fix: React 18.3's 19-style RefObject (mutable
     current: T) rejects RefObject<SVGPathElement | null> at the DOM
     `ref` and visx `innerRef` handoffs below. The null union is
     intentional (all readers guard it); this alias carries the same
     object where React assigns current before any read. */
  const pathDomRef = pathRef as unknown as React.Ref<SVGPathElement>;
