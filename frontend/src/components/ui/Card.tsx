...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('sentinel-card shadow-sentinel-card', className)}
      {...props}
    />
  )
);
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement> & { className?: string }>(
  ({ className, children, ...props }, ref) => (
    <h3 className={cn('text-lg font-semibold leading-none tracking-tight text-foreground', className)} {...props} ref={ref}>
      {children}
    </h3>
  )
);
