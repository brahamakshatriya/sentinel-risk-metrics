...
/* React Bits CRTWarp — TypeScript port for Sentinel auth pages.
   Source of truth: the JS + CSS variant from reactbits.dev (registry
   CRTWarp-JS-CSS). Rendering logic, shaders, and performance protections
   (IntersectionObserver, document.hidden check, FPS cap, dpr cap, full
   dispose on unmount) are unchanged.
   Sentinel-only adjustments (documented):
   1. TypeScript typings (required by `npx tsc --noEmit`) + 'use client'.
   2. prefers-reduced-motion: renders one static frame (plus one on resize),
      no rAF loop — background stays atmospheric, animation stops.
   3. Pointer tracking listens on window instead of the container so the
      background wrapper can stay pointer-events:none (never blocks auth
      inputs/buttons/OAuth) while mouseReact keeps working via bubbled
      events; reset on document mouseleave. */
...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    // Reduced motion: freeze time and render a single static atmospheric
    // frame (re-rendered once on resize). No rAF loop, no animation.
    const prefersReducedMotion =
      typeof window !== 'undefined' &&
      typeof window.matchMedia === 'function' &&
      window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (prefersReducedMotion) pausedRef.current = true;
    ...
    const render = (now: number) => {
      // Reduced motion: draw at most one frame per call, never loop.
    ...
    // Window-level pointer tracking: the background wrapper stays
    // pointer-events:none (never blocks auth UI), while bubbled pointer
    // events still drive the subtle mouse reaction.
  ...
  return <div ref={containerRef} className={`crt-warp-container ${className || ''}`} style={style} />;
}
